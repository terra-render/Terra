...
TerraRay        terra_ray ( TerraFloat3* origin, TerraFloat3* direction );

TerraRay        terra_surface_ray ( const TerraShadingSurface* surface, TerraFloat3* point, TerraFloat3* direction, float sign );
...
TerraFloat4x4   terra_camera_rotation ( const TerraCamera* camera );

TerraLight*     terra_scene_pick_light ( const TerraScene* scene, float e, float* pdf );
TerraObject*    terra_scene_raycast ( TerraScene* scene, const TerraRay* ray, TerraShadingSurface* surface_out, TerraFloat3* intersection_point, size_t* triangle );
...
void terra_render ( const TerraCamera* camera, HTerraScene _scene, const TerraFramebuffer* framebuffer, size_t x, size_t y, size_t width, size_t height ) {
    ...
    TerraFloat4x4 camera_rotation = terra_camera_rotation ( camera );
    ...
    for ( size_t i = y; i < y + height; ++i ) {
        for ( size_t j = x; j < x + width; ++j ) {
            ...
            for ( size_t s = 0; s < spp; ++s ) {
                ...
                // Build camera ray
                TerraFloat3 ray_dir = terra_camera_pixel_dir ( camera, framebuffer, j, i, scene->opts.subpixel_jitter, r1, r2 );
...
TerraFloat3 terra_trace ( const TerraScene* scene, const TerraRay* primary_ray ) {
    ...
    for ( size_t bounce = 0; bounce <= scene->opts.bounces; ++bounce ) {
        ...
        TerraObject* object;
        object = terra_scene_raycast ( scene, &ray, &surface, &intersection_point, NULL );
        ...
        TerraFloat3 reflectance = terra_mulf3 ( &f, 1.f / pdf );
...
TerraFloat3 terra_compute_direct ( const TerraScene* scene, const TerraShadingSurface* surface, const TerraMaterial* material, const TerraFloat3* p, const TerraFloat3* wo ) {
    ...
    {
        TerraFloat3 intersection_point;
        TerraRay ray = terra_surface_ray ( surface, p, &wi, 1 );
...
TerraFloat3 terra_compute_direct_mis ( const TerraScene* scene, const TerraShadingSurface* surface, const TerraMaterial* material, const TerraFloat3* p, const TerraFloat3* wo ) {
    ...
    // Sample light
    {
        ...
        TerraShadingSurface light_surface;
        {
            TerraFloat3 intersection_point;
            TerraRay ray = terra_surface_ray ( surface, p, &wi, 1 );
            object = terra_scene_raycast ( scene, &ray, &light_surface, &intersection_point, NULL );
        }
