...
#define LINMATH_H_DEFINE_VEC(n)                                                     \
    ...
    static inline void vec##n##_mul_cwise(vec##n r, vec##n const a, vec##n const b) \
    {                                                                               \
        int i;                                                                      \
        for (i = 0; i < n; ++i)                                                     \
            r[i] = a[i] * b[i];                                                     \
...
#define vec2_sz (sizeof(vec2))
#define vec3_sz (sizeof(vec3))
#define vec4_sz (sizeof(vec4))
...
static inline void vec3_set1(vec3 r, float xyz) {
    r[0] = xyz;
    r[1] = xyz;
    r[2] = xyz;
}
...
static inline void vec4_set1(vec4 r, float xyzw) {
    r[0] = xyzw;
    r[1] = xyzw;
    r[2] = xyzw;
    r[3] = xyzw;
...
static inline void vec2_store(vec2 r, vec2 a) {
    int i;
...
static inline void vec3_store(vec3 r, vec3 a) {
    int i;
...
static inline void vec4_store(vec4 r, vec4 a) {
    int i;
...
static inline void mat4_mul(mat4 M, const mat4 a, const mat4 b) {
    mat4 temp;
    int k, r, c;
...
static inline void quat_from_vec3(quat r, vec3 v) {
    r[0] = v[0];
    r[1] = v[1];
    r[2] = v[2];
    r[3] = 0.f;
...
#endif // _linear_algebra_types_h_